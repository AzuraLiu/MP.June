...
LASapwood<-aggregate(LA$Sapwood, by = list(LA$DOY, LA$Block),FUN = "sum")%>%
  mutate("ID"="LA")
names(LASapwood)[1]<-"DOY"
names(LASapwood)[2]<-"Block"
names(LASapwood)[3]<-"Sapwood"
...
LBSapwood<-aggregate(LB$Sapwood, by = list(LB$DOY, LB$Block),FUN = "sum")%>%
  mutate("ID"="LB")
names(LBSapwood)[1]<-"DOY"
names(LBSapwood)[2]<-"Block"
names(LBSapwood)[3]<-"Sapwood"
...
LCSapwood<-aggregate(LC$Sapwood, by = list(LC$DOY, LC$Block),FUN = "sum")%>%
  mutate("ID"="LC")
names(LCSapwood)[1]<-"DOY"
names(LCSapwood)[2]<-"Block"
names(LCSapwood)[3]<-"Sapwood"
...
LOSapwood<-aggregate(LO$Sapwood, by = list(LO$DOY, LO$Block),FUN = "sum")%>%
  mutate("ID"="LO")
names(LOSapwood)[1]<-"DOY"
names(LOSapwood)[2]<-"Block"
names(LOSapwood)[3]<-"Sapwood"
...
HASapwood<-aggregate(HA$Sapwood, by = list(HA$DOY, HA$Block),FUN = "sum")%>%
  mutate("ID"="HA")
names(HASapwood)[1]<-"DOY"
names(HASapwood)[2]<-"Block"
names(HASapwood)[3]<-"Sapwood"
...
HBSapwood<-aggregate(HB$Sapwood, by = list(HB$DOY, HB$Block),FUN = "sum")%>%
  mutate("ID"="HB")
names(HBSapwood)[1]<-"DOY"
names(HBSapwood)[2]<-"Block"
names(HBSapwood)[3]<-"Sapwood"
...
HCSapwood<-aggregate(HC$Sapwood, by = list(HC$DOY, HC$Block),FUN = "sum")%>%
  mutate("ID"="HC")
names(HCSapwood)[1]<-"DOY"
names(HCSapwood)[2]<-"Block"
names(HCSapwood)[3]<-"Sapwood"
...
HOSapwood<-aggregate(HO$Sapwood, by = list(HO$DOY, HO$Block),FUN = "sum")%>%
  mutate("ID"="HO")
names(HOSapwood)[1]<-"DOY"
names(HOSapwood)[2]<-"Block"
names(HOSapwood)[3]<-"Sapwood"
...
PlotLevel$GroundArea<- case_when(
    PlotLevel$Density == "H"  ~ 134.505 ,
    PlotLevel$Density == "L"  ~404.43 )


PlotLevel<-PlotLevel%>%
  mutate(SapwoodPerGroundArea=Sapwood/GroundArea)
...
```{r}
getwd()
PlotLevelSapwood<-read_xlsx( "../Data/Processed/Sapwood/PlotLevelSapwood.xlsx")


#write_xlsx(PerGround, "../Data/Processed/Sapwood/SapwoodPerGround.xlsx" )

  
 #PerGround<-PerGround%>%pivot_longer(cols= 2:9, names_to = "ID", values_to = "Area")
PlotLevelSapwood<-aggregate(PlotLevelSapwood$SapwoodPerGroundArea, by=list(PlotLevelSapwood$DOY,PlotLevelSapwood$ID), FUN = "sum")
names(PlotLevelSapwood)[1]<-"DOY"
names(PlotLevelSapwood)[2]<-"ID"
names(PlotLevelSapwood)[3]<-"Sapwood"

PlotLevelSapwood<-PlotLevelSapwood%>%mutate(Density=substr(PlotLevelSapwood$ID,1,1))

PlotLevelSapwood<-PlotLevelSapwood%>%mutate(Genotype=str_sub(PlotLevelSapwood$ID,-1,-1))



PlotLevelSapwoodArea<-ggplot(PlotLevelSapwood, aes(x=DOY, y=Sapwood, color=Genotype, linetype=Density))+
  scale_x_continuous()+
  geom_line(size=3)+
  xlab("DOY")+ylab("Sapwood (cm^2/m^2)")+
...
ggsave("../Data/Processed/Sapwood/SapwoodPerGroundLegend.jpeg", PlotLevelSapwoodArea, height = 7, width = 10, units = "in", dpi = 500)
```
