...
getwd()
Sapflow<-read_xlsx("../Data/Processed/Daytime/sapflow/Join_4Blocks.xlsx")%>%mutate("ratio"=Increment/Sapflow)
```

```{r}
m0<-aov(ratio~Density*Geno+(1|Block), data = Sapflow)
...
```{r}
m0<-aov(~Density*Geno+(1|DOY)+(1|Block), data = Sapflow)
TukeyHSD(m0)
...
m0<-aov(SapflowPerGroundArea~ID*DOY+(1|Block), data = Sapflow,type="II")
TukeyHSD(m0)
anova(m0)
m0
emmeans(m0, ~Density:Geno,data=Sapflow)

MOD<-Sapflow[c(2,4,5,6,7,8,10,13,14,15)]

Weekly<-aggregate(MOD, by=list(MOD$Week,MOD$ID,MOD$Block), FUN = mean)


names(Weekly)[1]<-"Week"
names(Weekly)[2]<-"ID"

Week<-Weekly[c(1,2,4,6,7,8,9,10,12)]

pwc <- Week %>% 
  emmeans_test(
    SapflowPerGroundArea ~ ID, covariate = REW,
    p.adjust.method = "bonferroni"
    )
pwc
library(ggpubr)
pwc <- pwc %>% add_xy_position(x = "REW", fun = "mean_se")
ggline(get_emmeans(pwc), x = "ID", y = "emmean") +
...
df <- apply(pwc,2,as.character)
write.table(pwc , file = "../Data/Processed/Daytime/sapflow/pwc.csv")

res.aov <- WeeklyAve %>% 
  anova_test(SapflowPerGroundArea ~ REW+VPD + Density*Genotype*Week+(1|Block))
