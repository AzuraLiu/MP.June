...
Sapflux<-ggplot(data=Means, aes(x=Genotype,y=Sapflux,fill=Density))+
  ...
  geom_errorbar(aes(ymin=Sapflux-40, ymax=Sapflux+40), width=.2,
...
Sapwood<-ggplot(data=Means, aes(x=Genotype,y=Sapwood,fill=Density))+
  ...
  geom_errorbar(aes(ymin=Sapwood-0.0878, ymax=Sapwood+0.0878), width=.2,
                 position=position_dodge(.9)) +
  theme(axis.title.y = element_text(size = 20))+ 
  theme(axis.title.x = element_blank())+
  theme(legend.text = element_text(size = 20))+ 
...
SapwoodIncrement<-ggplot(data=Means, aes(x=Genotype,y=Means$`Sapwood Increment`,fill=Density))+
  ...
  geom_errorbar(aes(ymin=Means$`Sapwood Increment`-0.189, ymax=Means$`Sapwood Increment`+0.189), width=.2,
                 position=position_dodge(.9)) +
  theme(axis.title.y = element_text(size = 20))+ 
...
Sapflow<-ggplot(data=Means, aes(x=Genotype,y=Sapflow,fill=Density))+
  geom_bar(stat="identity",position=position_dodge())+
  ylab("Sapflow(L/m^2)")+
  geom_errorbar(aes(ymin=Sapflow-0.0275, ymax=Sapflow+0.0275), width=.2,
                 position=position_dodge(.9)) +
  theme(axis.title.y = element_text(size = 20))+ 
  theme(axis.title.x = element_text(size=20))+
  theme(legend.text = element_text(size = 20))+ 
...
Sapflow
Comparison<-ggarrange(Sapflux,Sapwood,SapwoodIncrement,Sapflow, common.legend = TRUE,
          labels = c("Sapflux", "Sapwood Area","Sapwood Increment","Sapflow"),font.label = list(size = 20),hjust=-0.5,
